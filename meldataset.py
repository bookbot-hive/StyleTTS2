...
def preprocess(wave, to_melspec_fn):
    wave_tensor = torch.from_numpy(wave).float()
    mel_tensor = to_melspec_fn(wave_tensor)
...
class FilePathDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        print(f"MelDataset: sr={sr}, spec_params={spect_params}")
        ...
        self.ptexts = [t.split("|")[idx] for t in tl if len(t) > 8]
    ...
    def __getitem__(self, idx):
        ...
        while len(ps.split("#")) < self.min_length or len(ps.split("#")) > self.max_length:
